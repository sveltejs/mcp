...
4. **Confirmed false positive?** Silence one suggestion with a `// svelte-mcp-ignore <code>` (script) or `<!-- svelte-mcp-ignore <code> -->` (markup) comment on the line above, then re-run the autofixer. Codes are listed in the autofixer's reply if you write an unknown one.
